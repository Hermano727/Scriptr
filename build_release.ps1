...
if (Test-Path $pngPath) {
    ...
    foreach ($sz in $sizes) {
        ...
        # AND mask — all transparent (0 = use XOR with alpha blending)
    ...
    # ICO file layout: ICONDIR + N×ICONDIRENTRY + N×DIB blob
    $count      = $sizes.Count
    $dirBytes   = 6 + $count * 16      # header + all directory entries
    $fs         = [System.IO.File]::Create($icoPath)

    # ICONDIR header
    $fs.Write([byte[]](0x00, 0x00, 0x01, 0x00, [byte]$count, 0x00), 0, 6)

    # ICONDIRENTRY for each frame (offsets computed sequentially)
    $offset = $dirBytes
    for ($i = 0; $i -lt $count; $i++) {
        $sz   = [byte]$sizes[$i]
        $len  = $frames[$i].Length
        $entry = [byte[]](
            $sz, $sz, 0x00, 0x00,          # width, height, colorCount, reserved
            0x01, 0x00,                     # planes = 1
...
Write-Host "`nPublishing Scriptr.Stub..." -ForegroundColor Cyan
...
$stubKB = [math]::Round((Get-Item "$distDir\Scriptr.Stub.exe").Length / 1MB, 1)
Write-Host "  Scriptr.Stub.exe  -> dist\  ($stubKB MB)" -ForegroundColor Green

# ── Publish Scriptr.Gui ───────────────────────────────────────────────────────

Write-Host "`nPublishing Scriptr.Gui..." -ForegroundColor Cyan
$guiTmp = "$distDir\_gui"
...
Write-Host "`n── dist\ ─────────────────────────────────────────────" -ForegroundColor Yellow
Get-ChildItem $distDir | Sort-Object Name | ForEach-Object {
    $mb = "{0:N1} MB" -f ($_.Length / 1MB)
    Write-Host ("  {0,-28} {1,8}" -f $_.Name, $mb)
}
Write-Host "──────────────────────────────────────────────────────" -ForegroundColor Yellow
Write-Host "Release build complete. Both executables are standalone — no .NET runtime required on target machine." -ForegroundColor Green
